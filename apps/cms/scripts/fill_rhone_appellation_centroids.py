...
TARGET_SUBREGION_SLUGS = {"rhone-nord", "rhone-sud"}

EXPLICIT_ALIASES = {
    "crozes-hermitage": {"crozes-hermitage-ou-crozes-ermitage"},
    "hermitage": {"hermitage-ou-ermitage-ou-l-hermitage-ou-l-ermitage"},
    "muscat-beaumes-de-venise": {"muscat-de-beaumes-de-venise"},
    "rasteau-vdn": {"rasteau-vin-doux-naturel"},
}
...
def main() -> None:
    ...
    subregion_ids = [row["id"] for row in subregions]
    if not subregion_ids:
        print("✓ 0 appellations Rhone mises a jour (aucune subregion cible)")
        return
    ...
    if not appellation_ids:
        print("✓ 0 appellations Rhone mises a jour (aucun lien)")
        return
    ...
    print(f"✓ {updated} appellations mises a jour (Rhone Nord + Sud)")
    if unresolved:
